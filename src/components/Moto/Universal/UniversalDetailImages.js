...
const UniversalDetailImages = ({ myItem, imageotherFileList }) => {
  const [imageModal, setImageModal] = useState({
    previewVisible: false,
    previewImage: "",
  });

  if (isEmpty(imageotherFileList)) return null;
      ...
      <Row gutter={[8, 8]} justify="start">
        {imageotherFileList.map((item, index) => (
          <Col key={index} span={8}>
            <div
              className="moto-auction-thumb-image gx-card-widget"
...
export default UniversalDetailImages;
